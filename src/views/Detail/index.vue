          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl
                ...
                :key="spuSaleAttrItem.baseSaleAttrId"
              >
                <dt class="title">{{ spuSaleAttrItem.saleAttrName }}</dt>
                <dd
                  changepirce="0"
                  :class="{ active: +spuSaleAttrValueItem.isChecked }"
                  v-for="(
                    spuSaleAttrValueItem, index
                  ) in spuSaleAttrItem.spuSaleAttrValueList"
                  :key="index"
                  @click="
                    changeAct(
                  ...
                  "
                >
                  {{ spuSaleAttrValueItem.saleAttrValueName }}
                </dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input
                  ...
                  @change="checkNumb"
                />
                <a class="plus" @click="numb++">+</a>
                <a class="mins" @click="numb <= 1 ? (numb = 1) : numb--">-</a>
              </div>
              <div class="add">
...
export default {
  ...
  methods: {
    ...mapActions("detail", ["reqDetailInfo"]),
    changeAct(targ, list) {
      list.forEach((item) => {
        item.isChecked = 0;
      });
    ...
    checkNumb() {
      this.numb = +this.numb;
      if (isNaN(this.numb) || this.numb < 1) {
        this.numb = 1;
      } else {
    ...
    addToCar() {
      // reqAddToCart(this.$route.params.skuId, this.numb)
      //   .then((result) => {
      //     console.log(result);
      //   })
      //   .catch((error) => {
      ...
      let that = this;
      (async function atc(that) {
        let result = await reqAddToCart(
          that.$route.params.skuId,
          that.numb,
          getUUID()
        );
        if (result.code === 200) {
          ...
          sessionStorage.setItem("SKUINFO", JSON.stringify(that.skuInfo));
        } else {
          console.log("添加失败");
        }
      })(that);
  ...
  mounted() {
    this.reqDetailInfo(this.$route.params.skuId);
  },
};
</script>
